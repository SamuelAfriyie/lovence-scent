...
const ProductCard: React.FC<ProductProps> = ({ name, price, imageUrl, className }) => {
  return (
    <div className={cn(" p-4 transition duration-300", className)}>
      ...
      <div className="text-center flex flex-col items-center">
        <h3 className="text-base font-semibold text-gray-600 mb-1 hover:underline">{name} <br /><span className='font-sans'>{"By Kelvin Klein"}</span></h3>
        <div className='w-full flex items-center space-x-4 justify-center'>
          <p className="text-xl font-black text-black"><span className='text-xs'>GH₵</span>{price}</p>
          <div className='px-4 ring-1 ring-inset rounded-full ring-gray-500 w-fit py-1 hover:ring-2'>
...
export const ProductCardV2 = ({
  ...
  return (
    <div className={cn(
      "group relative p-4 transition-all duration-300 ring-inset hover:ring-2 hover:ring-blue-500 bg-white rounded-lg",
      isList ? "flex flex-row items-center gap-6" : "flex flex-col",
      className
    )}>
      {/* Discount Badge */}
      {discount > 0 && (
        <span className="absolute top-2 left-2 bg-red-500 text-white text-[10px] font-bold font-serif px-2 py-1 rounded-full z-10">
          -{discount}%
        </span>
      )}
        ...
        <div className={cn(
          ...
          >
            <div className="relative">
              ...
              {isAddedToCart && (
                <span className="absolute -top-3 -right-3 flex h-4 w-4 items-center justify-center rounded-full bg-red-600 text-[9px] font-black text-white ring-2 ring-white">
                  {isAddedToCart.qty}
                </span>
              )}
            </div>
